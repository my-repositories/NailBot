...
* [x] Validate at startup (fail fast or enter restricted mode).
* [x] Periodically re-check (e.g., every 24 hours) with backoff on failures.
* [x] Never log the full license key (redact/hash).

### Verification model (choose one; document in `docs/DEPLOY.md`)

* [x] **Offline signed token** (recommended):
  * [x] License is a signed payload (e.g., Ed25519) containing `customer_id`, `expires_at`, optional `features`.
  * [x] App embeds public key and verifies signature locally.
...
* [x] Define behavior:
  * [x] Hard stop at boot (strict)
  * [ ] Or allow read-only / limited mode (graceful) and block booking/admin actions
* [x] User-facing messaging is friendly; admin notification includes next steps.
...
- Runtime now enforces strict on-prem startup check for `LICENSE_KEY`.
- SaaS startup path does not gate execution on license settings.
- Full cryptographic verification and periodic re-check worker are tracked as follow-up implementation.
